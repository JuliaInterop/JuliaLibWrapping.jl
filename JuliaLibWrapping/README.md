...
The Python target separates generated `ctypes` bindings in `_lowlevel.py` and
generated idiomatic wrappers in `_generated.py` from the author-editable public
API in `_facade.py`. The two generated modules are rewritten on every build;
`_facade.py` is created once. See the
