...
_lib.mylib_scale_2.argtypes = [CVector_borrowed_Float64, CVector_borrowed_Float64]
_lib.mylib_scale_2.restype = JLWResult_CVector_owned_Float64
def mylib_scale_2(x, y):
    """Scale `x` by the entries of `y`."""
    _result = _lib.mylib_scale_2(x, y)
    if _result.status.code != 0:
        _msg = bytes(_result.status.message).rstrip(b"\x00").decode("utf-8", errors="replace")
        raise JLWError(_result.status.code, _msg)
    return _result
