...
@testset "JuliaLibWrapping.jl" begin
    ...
    @testset "api arity overloads" begin
        # Two C symbols share the sidecar name `scale`: the façade defines
        # `scale` once, dispatching on the positional count. One arity takes
        # keywords and the other takes none.
        info = read_abi_info("bindinginfo_arity.json")
        meta = JuliaLibWrapping.read_api_metadata(joinpath(@__DIR__, "arity.jlw.json")).exports
        @test isnothing(JuliaLibWrapping.check_metadata_consistency(info, meta))
        mktempdir() do dir
            dest = PythonTarget(dir, "mylib_py", "mylib")
            write_wrapper(dest, info; api_metadata = meta)
            facade_path = joinpath(dir, "mylib_py", "_facade.py")
            facade = read(facade_path, String)
            @test occursin("def _scale_1(x, *, factor=2.0, label):", facade)
            @test occursin("def _scale_2(x, y):", facade)
            @test occursin("def scale(*args, **kwargs):", facade)
            @test occursin(
                "f\"scale() takes 1 or 2 positional arguments but {len(args)} were given\"",
                facade
            )
            # Both docstrings, in ascending arity order.
            @test occursin("\"\"\"Scale every entry.\n\n    Scale `x` by the entries of `y`.\"\"\"", facade)
            # The public name is exported once; the private wrappers are not.
            all_line = onlymatch(l -> startswith(l, "__all__ = "), split(facade, '\n'))
            @test count(==("\"scale\""), split(all_line[(findfirst('[', all_line) + 1):(end - 1)], ", ")) == 1
            @test !occursin("\"_scale_1\"", all_line)
            @test facade == read(joinpath(@__DIR__, "expected_arity_facade.py"), String)

            bindings_path = joinpath(dir, "mylib_py", "_lowlevel.py")
            bindings = read(bindings_path, String)
            @test bindings == read(joinpath(@__DIR__, "expected_arity_lowlevel.py"), String)
            ...
            if !isnothing(python3)
                ...
                # The dispatcher itself, over stub wrappers: the positional
                # count picks the arity, and any other count is a TypeError.
                dispatcher = facade[findfirst("def scale(*args, **kwargs):", facade)[1]:(findfirst("__all__ = ", facade)[1] - 1)]
                script = joinpath(dir, "dispatch_check.py")
                write(
                    script, join(
                        [
                            "def _scale_1(*args, **kwargs):",
                            "    return \"arity1\"",
                            "",
                            "def _scale_2(*args, **kwargs):",
                            "    return \"arity2\"",
                            "",
                            dispatcher,
                            "assert scale(1) == \"arity1\"",
                            "assert scale(1, 2) == \"arity2\"",
                            "try:",
                            "    scale()",
                            "except TypeError as e:",
                            "    assert \"1 or 2\" in str(e), str(e)",
                            "else:",
                            "    raise AssertionError(\"scale() did not raise TypeError\")",
                        ], "\n"
                    )
                )
                @test success(run(pipeline(`$python3 $script`; stderr = devnull, stdout = devnull); wait = true))
            elseif haskey(ENV, "CI")
                error("python3 not found on PATH; required on CI to validate the emitted wrapper")
            end
        end
    ...
    @testset "duplicate façade names" begin
        # Two entrypoints claiming one Python name at the same positional
        # arity: nothing at a call site could choose between them.
        info = read_abi_info("bindinginfo_api_scale.json")
