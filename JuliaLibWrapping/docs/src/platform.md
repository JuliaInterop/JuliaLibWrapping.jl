...
language, so it is hoisted out of the Python target into
...
target calls `bundle_libdir(os_kernel)` rather than inlining its own
per-OS ternary.
...
A future binding target (a MATLAB `.mex` target via
...
- **Consume, don't re-derive, the target-independent facts.** Which
  ...
  per-OS ternary in a new target.
- **Own your target's loader idiom.** How a target's *runtime* locates and
  ...
  target, not in `platform.jl`: Python's answer is `ctypes.CDLL` plus
