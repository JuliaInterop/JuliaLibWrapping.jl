"""
    PythonTarget(dir, package_name, library_basename;
                 ...
                 privatized = false, coerce_arrays = false)
...
`coerce_arrays` selects how a generated wrapper prepares an array argument.
With `false`, the default, the array is handed to the carrier's `from_numpy`,
which raises `ValueError` when the dtype or memory order differs from the
library's. With `true` the wrapper first applies
`np.asfortranarray(x, dtype=...)` to an array of two or more dimensions and
`np.ascontiguousarray(x, dtype=...)` to a vector. Input that does not already
match is copied without notice, so the option is off by default.
"""
...
PythonTarget(
    ...
    privatized::Bool = false, coerce_arrays::Bool = false
) =
    PythonTarget(
...
function Base.show(io::IO, t::PythonTarget)
    ...
    t.coerce_arrays && print(io, "; coerce_arrays = true")
    return print(io, ")")
end
...
function write_wrapper(
    ...
    # Emit into a temporary file and rename on success, as for `_generated.py`
    ...
    # `_generated.py` holds the idiomatic wrappers and is rewritten on every
    # call, through a temporary file for the same reason as `_lowlevel.py`.
    generated_path = joinpath(pkgdir, "_generated.py")
    _write_atomically(generated_path, pkgdir) do f
        _write_generated(f, dest, abi_info, typedict, needs_jlwerror, api_metadata, api_enums)
    end

    # `_facade.py` is the author-editable public API, a starter that imports
    # everything `_generated.py` defines. It is created only if absent, so
    # hand-written code in it survives later builds.
    ...
    if !isfile(facade_path)
        ...
        _write_atomically(facade_path, pkgdir) do f
            _write_facade_starter(f, dest)
        end
    end

    # `_generated.py` always defines `__all__`, so `_facade.py` does too.
...
- `(kind=:carray, classname=…, ndim=…, dtype=…)` — a borrowed `CArray`; wrap
  with `<class>.from_numpy(name)`. `ndim` and the numpy dtype name `dtype`
  select the coercion in [`_facade_arg_conversions`](@ref).
...
function _facade_classify_arg(
    ...
    elseif t isa StructDesc
        ...
        if !isnothing(cainfo)
            ...
            return (
                kind = :carray, classname = typedict[arg.type],
                ndim = cainfo.ndim, dtype = cainfo.dtype,
            )
        elseif !isnothing(csinfo)
...
"""
    _facade_arg_conversions(f, argnames, arg_classes, coerce_arrays) -> Vector{String}

Emit conversions for non-primitive arguments and return the low-level call
arguments. With `coerce_arrays`, an array argument passes through
`np.asfortranarray` (rank two and higher) or `np.ascontiguousarray` (vectors)
with the carrier's dtype before `from_numpy`, so a mismatched dtype or memory
order is copied rather than rejected.
"""
function _facade_arg_conversions(
        f::IO, argnames::Vector{String}, arg_classes, coerce_arrays::Bool
    )
    call_args = String[]
    for (name, cls) in zip(argnames, arg_classes)
        ...
        elseif cls.kind === :carray
            ...
            arg = name
            if coerce_arrays
                coercion = cls.ndim >= 2 ? "np.asfortranarray" : "np.ascontiguousarray"
                arg = coercion * "(" * name * ", dtype=\"" * cls.dtype * "\")"
            end
            println(f, "    ", local_, " = ", cls.classname, ".from_numpy(", arg, ")")
...
"""
    _emit_facade_autowrapper(f, method, plan, coerce_arrays)

Emit an automatically classified façade wrapper. `coerce_arrays` is as in
[`_facade_arg_conversions`](@ref).
"""
function _emit_facade_autowrapper(f::IO, method::MethodDesc, plan, coerce_arrays::Bool)
    arg_names_seen = Set{String}()
    argnames = String[
    ...
    println(f, "def ", method.symbol, "(", join(argnames, ", "), "):")
    call_args = _facade_arg_conversions(f, argnames, plan.args, coerce_arrays)
...
"""
    _emit_facade_api_autowrapper(f, method, plan, coerce_arrays)
...
shape differ. `coerce_arrays` is as in [`_facade_arg_conversions`](@ref).
"""
function _emit_facade_api_autowrapper(f::IO, method::MethodDesc, plan, coerce_arrays::Bool)
...
Write the starter `_facade.py`: a module that imports everything
`_generated.py` defines and copies its `__all__`. It is written once and then
left to the author.
"""
function _write_facade_starter(f::IO, dest::PythonTarget)
    println(f, "\"\"\"", dest.package_name, " public API.")
    println(f)
    println(f, "JuliaLibWrapping writes this file once and never overwrites it. The")
    println(f, "generated wrappers live in `_generated.py` and the ctypes bindings in")
    println(f, "`_lowlevel.py`; both are rewritten on every build. Everything defined in")
    println(f, "`_generated` is imported here, so a package with no hand-written layer")
    println(f, "exposes the generated API unchanged.")
    println(f)
    println(f, "Add, override, or hide names below:")
    println(f)
    println(f, "- to add a function, define it here and append its name to `__all__`;")
    println(f, "- to override a generated function under its public name, define it here")
    println(f, "  after the import; the generated one stays reachable as")
    println(f, "  `_generated.<name>`;")
    println(f, "- to hide a generated name, remove it from `__all__`.")
    println(f)
    println(f, "`__init__.py` re-exports whatever `__all__` lists.")
    println(f, "\"\"\"")
    println(f, "from . import _generated")
    println(f, "from ._generated import *  # noqa: F401,F403")
    println(f)
    return println(f, "__all__ = list(_generated.__all__)")
end

"""
    _write_generated(f, dest, abi_info, typedict, needs_jlwerror, api_metadata, api_enums)

Write `_generated.py`: the idiomatic wrappers over `_lowlevel`. Every
entrypoint whose arguments and return classify ([`_facade_plan`](@ref)) gets a
wrapper taking and returning Python objects; anything else is re-exported from
`_lowlevel` with a `TODO` comment. The module is rewritten on every
[`write_wrapper`](@ref) call.
"""
function _write_generated(
        f::IO, dest::PythonTarget, abi_info::ABIInfo,
    ...
    println(f, "\"\"\"", dest.package_name, " generated façade.")
    println(f)
    println(f, "Idiomatic wrappers over `_lowlevel`: functions whose arguments and return")
    println(f, "are all recognized (primitives, carriers such as `CArray`, `CString`,")
    println(f, "`CDict`, `COpt`, enums, and `JLWResult`/`JLWStatus` returns) accept and")
    println(f, "return Python objects (numpy arrays, `str`, `dict`, ...). Anything else is")
    println(f, "re-exported from `_lowlevel` with a `TODO` comment naming what needs")
    println(f, "hand-wrapping.")
    println(f)
    println(f, "This file is regenerated on every build. Do not edit it; put hand-written")
    println(f, "code in `_facade.py`, which imports everything defined here.")
    if dest.coerce_arrays
        println(f)
        println(f, "Array arguments are coerced before the call: `np.asfortranarray(x, dtype=...)`")
        println(f, "for arrays of two or more dimensions and `np.ascontiguousarray(x, dtype=...)`")
        println(f, "for vectors. Input with the wrong dtype or memory order is copied silently.")
    end
    println(f, "\"\"\"")
    ...
    for (method, plan) in zip(entrypoints, plans)
        if plan.category === :auto
            _emit_facade_autowrapper(f, method, plan, dest.coerce_arrays)
        elseif plan.category === :api_auto
            _emit_facade_api_autowrapper(f, method, plan, dest.coerce_arrays)
        end
    end
