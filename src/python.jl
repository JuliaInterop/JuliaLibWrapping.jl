...
function write_wrapper(dest::PythonTarget, abi_info::ABIInfo)
    ...
    # Collect the public names for re-export. Tuple-shaped structs do not
    # emit a class so they are skipped here.
...
function _write_facade_stub(f::IO, dest::PythonTarget, exported_names::AbstractVector{<:AbstractString})
    println(f, "\"\"\"", dest.package_name, " idiomatic façade.")
    println(f)
    println(f, "This file is generated **once** by JuliaLibWrapping as a starter stub")
    println(f, "that re-exports everything from `_lowlevel`. Edit it freely —")
    println(f, "JuliaLibWrapping will never overwrite it on subsequent runs. Delete")
    println(f, "it to regenerate.")
    println(f)
    println(f, "The mechanical bindings live in `_lowlevel.py` and are regenerated on")
    println(f, "every `write_wrapper` call.")
    println(f, "\"\"\"")
    if isempty(exported_names)
        println(f, "from . import _lowlevel  # noqa: F401")
        println(f)
